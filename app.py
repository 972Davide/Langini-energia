...
st.markdown("""
    ...
    @media (max-width: 768px) { [data-testid="column"] { width: 100% !important; flex: 1 1 100% !important; margin-bottom: 15px !important; } }
    </style>
    """, unsafe_allow_html=True)

# --- TESTATA ---
c_title, c_button = st.columns([0.85, 0.15])
with c_title: st.title("⚡ Langini: Intelligenza Energetica")
with c_button:
...
def carica_e_elabora():
    try:
        # Caricamento diretto dai link pubblicati
...
if df is not None and not df.empty:
    ...
    # --- CALCOLI ENERGETICI (PROIEZIONE A FINIRE) ---
    ...
    ee3.markdown(f'<div class="big-metric">Stima Annua (A finire)</div><div class="big-value">{stima_annua_finire:.1f} kWh</div><div class="small-trend">Media 2026: {media_giornaliera:.2f} kWh/gg</div>', unsafe_allow_html=True)
    ...
    st.warning("Dati non disponibili. Verifica la pubblicazione dei file Google Sheets.")
