...
def carica_e_elabora():
    try:
        ...
        # --- FILTRI ANTI-OUTLIER (Sanity Check) ---
        # Rimuoviamo righe con valori fisicamente impossibili
        df = df[
            (df['Temperatura'] > -50) & (df['Temperatura'] < 60) &
            (df['Pressione Locale'] > 800) & (df['Pressione Locale'] < 1200) &
            (df['Vento (m/s)'] >= 0) & (df['Vento (m/s)'] < 100) &
            (df['Watt'] >= 0) & (df['Watt'] < 15000) # Adatta il 15000 alla tua max potenza
        ]
