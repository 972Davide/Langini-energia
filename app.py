...
def carica_dati():
    try:
        ...
        r1 = requests.get(URL_METEO).content
        r2 = requests.get(URL_EOLICO).content
        
        # Leggi i CSV
        df1 = pd.read_csv(io.BytesIO(r1))
        df2 = pd.read_csv(io.BytesIO(r2))
        
        # Rinominiamo la prima colonna in "Tempo" (A1 deve chiamarsi Tempo)
...
if isinstance(risultato, str):
    ...
    st.write("Verifica che la cella A1 di entrambi i fogli contenga esattamente la parola 'Tempo'.")
else:
    st.success("Dati caricati!")
    st.dataframe(risultato.tail())
    # Grafico veloce
    st.line_chart(risultato.set_index("Tempo"))
