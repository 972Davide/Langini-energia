...
# --- CONFIGURAZIONE ---
PREZZO_GSE_KW = 0.15 
# Utilizziamo i link ESATTI che hai pubblicato negli screenshot
...
def carica_e_elabora():
    try:
        # Caricamento forzato: se il file è sporco all'inizio, saltiamo le prime righe
        df1 = pd.read_csv(URL_METEO)
        df2 = pd.read_csv(URL_EOLICO)
        
        # Pulizia: forziamo la prima colonna a essere la data/tempo
        for df in [df1, df2]:
        ...
        # Unione basata sul tempo
        df = pd.merge(df1, df2, on='Tempo', how='inner')
        
        # Pulizia dati numerici: gestisce punti, virgole e valori mancanti
...
if df is not None and not df.empty:
    ...
    st.warning("⚠️ Dati non trovati. Assicurati che i link Google Sheets siano pubblici e in formato CSV.")
